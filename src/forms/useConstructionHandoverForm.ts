...
function useConstructionHandoverFormValues(
  ...
  return {
    ...
    constructionHandoverFinancing: (constructionHandover.constructionHandoverFinancing ?? []).map((item) => ({
    ...
    totalCost: constructionHandover.totalCost !== null ? `${constructionHandover.totalCost}` : '',
  };
}
