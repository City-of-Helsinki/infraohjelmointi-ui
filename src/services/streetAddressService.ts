 ...
 * @returns The complete search parameter for street number for the numeric
 * substring found at the end of the address, or '' if no digits are present.
 */
function getStreetNumber(input: string) {
  const matches = /\d{1,5}$/.exec(input);
  if (matches) {
    return `AND(osoitenumero=%27${matches[0]}%27)`;
  }
  return '';
}
...
export async function fetchAddressData(address: string) {
  ...
  url += `&CQL_FILTER=((katunimi%20ILIKE%20%27${streetName}%25%27%20OR%20gatan%20ILIKE%20%27${streetName}%25%27)${streetNumber})`;
