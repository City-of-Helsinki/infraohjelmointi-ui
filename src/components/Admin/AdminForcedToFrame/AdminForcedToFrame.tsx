...
const AdminForcedToFrame = () => {
  ...
  const forcedToFrameUpdatedDate = moment(new Date(forcedToFrameState.lastModified)).format(
    'D.M.YYYY HH:mm:ss',
  );
  const forcedToFrameDataUpdatedDate = moment(
    new Date(forcedToFrameDataUpdated.lastModified),
  ).format('D.M.YYYY HH:mm:ss');
  const { isConfirmed } = useConfirmDialog();

  const handleForcedToFrameStatusChange = async () => {
    dispatch(
      setLoading({ text: 'Change forced to frame status', id: 'change-forced-to-frame-status' }),
    );
    try {
      const request = {
      ...
      dispatch(
        notifyError({
          message: 'forcedToFrameStatusUpdateError',
          type: 'notification',
          title: 'undefined',
        }),
      );
  ...
  const handleForcedToFrameDataUpdate = async () => {
    ...
    if (confirm !== false) {
      dispatch(
        setLoading({ text: 'Update all forced to frame data', id: 'update-forced-to-frame-data' }),
      );
      try {
  ...
  return (
    <div data-testid="admin-forcedtoframe">
      <Fieldset
        border
        heading={t(`adminFunctions.forcedtoframestate.viewState`) ?? ''}
        style={{ display: 'flex', flexDirection: 'row', gap: '60px' }}
      >
        <div>
          <p style={{ fontWeight: 'bold' }}>{t(`status`)}</p>
          <p>
            {forcedToFrameState.isOpen
              ? t(`adminFunctions.forcedtoframestate.open`)
              : t(`adminFunctions.forcedtoframestate.locked`)}
          </p>
        </div>
        <div>
          <p style={{ fontWeight: 'bold' }}>
            {t(`adminFunctions.forcedtoframestate.lastModified`)}
          </p>
          <p>{forcedToFrameUpdatedDate}</p>
        </div>
        <div>
          <p style={{ fontWeight: 'bold' }}>
            {t(`adminFunctions.forcedtoframestate.changeStatus`)}
          </p>
          <ToggleButton
            id={''}
            label={undefined}
            checked={forcedToFrameState.isOpen}
            onChange={handleForcedToFrameStatusChange}
          ></ToggleButton>
        </div>
      </Fieldset>
      <Fieldset
        border
        heading={t(`adminFunctions.forcedtoframestate.moveData`) ?? ''}
        style={{ display: 'flex', flexDirection: 'row', gap: '60px', marginTop: '30px' }}
      >
        <div>
          <p>{t(`adminFunctions.forcedtoframestate.updateDataInfo`)}</p>
          <p style={{ fontWeight: 'bold' }}>
            {t(`adminFunctions.forcedtoframestate.dataLastUpdated`)}
          </p>
          <p>{forcedToFrameDataUpdatedDate}</p>
        </div>
        <Button
          style={{ height: '50%', marginTop: '14px' }}
          onClick={handleForcedToFrameDataUpdate}
        >
          {t(`adminFunctions.forcedtoframestate.updateData`)}
        </Button>
      </Fieldset>
    </div>
  );
